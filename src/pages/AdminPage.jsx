  ...
  return (
    <div className={darkMode ? 'dark' : ''}>
      <div className="min-h-screen bg-theme-primary text-theme-primary transition-colors duration-300 flex flex-col">
        ...
        {/* ── Mobile Clean Tab Strip ── */}
        <div className="md:hidden w-full bg-theme-card/95 backdrop-blur-md border-b border-theme/60 px-4 flex gap-6 overflow-x-auto shrink-0 sticky top-0 z-20" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {TABS.map(tab => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 py-3 text-xs font-semibold uppercase tracking-wider whitespace-nowrap border-b-2 transition-all shrink-0 ${
                  active
              ...
              >
                ...
                {tab.count !== undefined && (
                  <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${
                    active ? 'bg-orange-500/15 text-orange-500' : 'bg-theme-muted/50 text-theme-muted'
                  }`}>
                    {tab.count}
                  </span>
                )}
